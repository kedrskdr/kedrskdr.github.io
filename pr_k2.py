...
ann.set_learning_rate(float(sys.argv[3]))
...
ann.randomize_weights(-1*float(sys.argv[4]),float(sys.argv[4]))
...
hit = [i for i in range(len(data)) if ext[i][-1]==1]
clear = [i  for i in range(len(data)) if ext[i][-1] == 0]
random.shuffle(clear)
clear = clear[:len(hit)*2]
indvec = hit + clear
random.shuffle(indvec)
vec_train = [data[i] for i in indvec[:int(len(indvec)*0.8)]]
res_train = [ext[i] for i in indvec[:int(len(indvec)*0.8)]]
vec_test = [data[i] for i in indvec[int(len(indvec)*0.8):]]
res_test = [ext[i] for i in indvec[int(len(indvec)*0.8):]]
